...
# Mantenemos exactamente la misma estructura de carga de datos
@st.cache_data
def cargar_datos():
...
# Conservamos el mismo título y estructura de inputs
st.title("Cotizador ThreatDown con Descuentos")

# Filtro 1: Selección del término (idéntico al original)
...
# --- Optimizaciones internas manteniendo flujo visible igual ---
cotizacion = []
productos_para_tabla_secundaria = []

for prod in seleccion:
    # Mantenemos la lógica original de obtención de precios
    ...
    # Misma validación de rangos
    df_rango = df_producto[
    ...
    if not df_rango.empty:
        ...
        # Inputs de descuentos idénticos (solo mejorado el layout)
        col1, col2, col3 = st.columns(3)
        with col1:
            item_disc = st.number_input(
                f"Descuento 'Item' (%) para '{prod}':",
                min_value=0.0, 
                max_value=100.0, 
                value=0.0
            )
        with col2:
            channel_disc = st.number_input(
                f"Descuento 'Channel Disc.' (%) para '{prod}':",
                min_value=0.0,
                max_value=100.0,
                value=0.0
            )
        with col3:
            deal_reg_disc = st.number_input(
                f"Descuento 'Deal Reg. Disc.' (%) para '{prod}':",
                min_value=0.0,
                max_value=100.0,
                value=0.0
            )
        
        # Mismos cálculos originales
...
# Sección de cotización idéntica en salida
if cotizacion:
...
# Tabla secundaria idéntica en funcionalidad y outputs
